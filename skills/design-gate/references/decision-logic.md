...
- One current refinement worth knowing when the tools verdict lands: if the tool surface has to be large, current guidance favors exposing code execution over an API rather than defining dozens of individual tools. The verdict does not change; the tool design under it should.

## What each signal changes

The verdict is the shape. The rest of the guidance is layered on top from the other signals.

- **Knowledge** comes from `q5`, with one override: a workflow never does RAG, because RAG puts a model in the retrieval path. For a workflow, knowledge is a deterministic lookup by key or query. Otherwise: `docs` → RAG over a vector store, or agentic search where the agent can afford to explore (current guidance treats these as a spectrum, not a single default), `live` → tools or APIs at query time, `records` → scoped, parameterized query tools over the system of record, `model` → no retrieval.
